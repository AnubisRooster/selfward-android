package com.theraipist.data.local.download
 ...
 * across process restarts by a stable title (`theraipist:<modelId>`) rather than a
 ...
 * it isn't unit-tested (no device in CI), matching [com.theraipist.data.local.llm.LlamaCppLocalService].
 */
class AndroidModelDownloader(private val context: Context) : ModelDownloader {
    ...
    private fun titleFor(model: LocalModel) = "theraipist:${model.id}"
