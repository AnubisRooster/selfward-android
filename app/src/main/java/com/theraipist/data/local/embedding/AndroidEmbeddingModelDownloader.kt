 ...
 * pair via the OS [DownloadManager], mirroring [com.theraipist.data.local.download.AndroidModelDownloader]'s
...
class AndroidEmbeddingModelDownloader(private val context: Context) : EmbeddingModelDownloader {
    ...
    private fun onnxAsset(model: EmbeddingModelSpec) = Asset(
        title = "theraipist:${model.id}:onnx",
        url = model.onnxUrl,
    ...
    private fun vocabAsset(model: EmbeddingModelSpec) = Asset(
        title = "theraipist:${model.id}:vocab",
        url = model.vocabUrl,
